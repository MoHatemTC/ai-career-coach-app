...
/**
 * The conversation. It is the Upload screen's main element, and lives here
 * rather than in that page so the transcript survives a route change and can be
 * mounted anywhere else without being reimplemented.
 *
 * The degraded path is the reason this is shared rather than copied. A second,
 * subtly different keyword router would be indistinguishable from the real
 * agent to anyone reading the transcript, which is exactly the failure the
 * product forbids.
 */
export function useConversation() {
  ...
  async function routeByKeyword(text: string): Promise<string> {
    ...
    if (!profile) {
      return "I need your profile first. Open the panel on the right and upload a CV, then ask me again.";
    }
      ...
      try {
        ...
        if (shouldRun) {
          ...
          reply = current
            ...
            : `${reply}\n\nI need your profile first though. Open the panel on the right and upload a CV.`;
        }
      } catch (caught) {
  ...
  return { profile, history, busy, send };
}
