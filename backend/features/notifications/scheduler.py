...
cron because it runs in-process with the FastAPI app — one thing to deploy, and
it behaves identically on the Windows machines this team develops on, where
cron does not exist.
...
local time for anyone outside the server's zone. So the trigger runs at the top
of every hour and `run_daily_dispatch()` only sends to users whose local hour
matches theirs right now. The per-user, per-local-day idempotency check in the
dispatcher is what keeps 24 ticks from producing 24 digests.

Scaling note: this is a single-process, in-memory scheduler. If the API is ever
run with more than one worker (`uvicorn --workers 2`), every worker gets its own
scheduler and the job fires N times. The idempotency check makes that safe
rather than catastrophic, but the correct fix is to run the scheduler as its own
single-replica process — see `scripts/run_scheduler.py`.
"""
...
def scheduler_enabled() -> bool:
    ...
    notifications at real addresses.
    """
...
def _tick() -> None:
    ...
    The request-scoped `get_db()` dependency is unavailable here — this runs on
    a background thread with no request — so the session is opened and closed
    explicitly.
    """
    db = SessionLocal()
    try:
...
def get_scheduler_status() -> dict:
    """Introspection for `GET /notifications/scheduler` and the settings page."""
