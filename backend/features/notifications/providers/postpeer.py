...
=============================================================================
 ACTION REQUIRED — VERIFY THE POSTPEER CONTRACT BEFORE ENABLING IN PROD
=============================================================================
I could not find Postpeer's API contract in this repository or its history
(grepped every branch: the only hits for "whatsapp"/"notif" anywhere are
.env.example's empty EMAIL_* block and docs/PRD.md). So rather than invent an
endpoint shape and have it silently 404, every vendor-specific detail below is
driven by environment variables with conservative defaults.

Get these four answers from Eng. Sara / the Postpeer dashboard, set the env
vars in `.env`, and this provider works without a code change:
...
If the real contract turns out not to fit this shape, `_build_request()` is
the only method to rewrite.
...
business-initiated by definition, so unless Postpeer resells a number under
its own approved template, this channel needs a template submitted and
approved before it can deliver anything.
...
class PostpeerWhatsAppProvider(NotificationProvider):
    ...
    def _build_request(self, payload: DigestPayload, recipient: str) -> tuple[str, dict, dict, dict]:
        """Return (url, headers, json_body, query_params).

        Isolated so that adapting to the real Postpeer contract is a one-method
        ...
        if self.template_name:
            ...
            # only the variable parts. Ordering here must match the placeholder
            # order in the approved template ({{1}}, {{2}}, {{3}}).
                ...
                "components": [
                    {
                        ...
                        "parameters": [
                            ...
                            {"type": "text", "text": f"{top.job.title} at {top.job.company}"},
                        ],
                    }
                ],
            }
    ...
    def send(self, payload: DigestPayload, recipient: str) -> DeliveryResult:
        ...
        try:
            ...
            # Network-level failure. Caller falls back to email.
            logger.warning("Postpeer request failed for %s: %s", payload.user_id, exc)
            return DeliveryResult.failed(self.channel, self.name, f"request failed: {exc}")

        if response.status_code >= 400:
            # Truncated: provider errors can be long HTML pages, and this
            # string lands in a DB column read by humans.
        ...
        try:
            ...
            # 2xx with a non-JSON body still counts as delivered; we just have
            # no id to record.
            pass
