...
Why an interface at all, when the brief names one vendor? Because the brief
...
signature. It also means the Postpeer specifics stay quarantined in one file
when its API is confirmed.
"""
...
class NotificationProvider(abc.ABC):
    ...
    def is_configured(self) -> bool:
        ...
        Checked *before* a run so a misconfigured provider degrades to the
    ...
    def send(self, payload: DigestPayload, recipient: str) -> DeliveryResult:
        ...
        `DeliveryResult.failed(...)` instead. A raise here aborts the nightly
